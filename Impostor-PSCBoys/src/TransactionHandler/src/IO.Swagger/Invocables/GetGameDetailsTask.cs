...
{
    public class GetGameDetailsTask : IInvocable, IInvocableWithPayload<string>
    {
        private readonly ITransactionManager _transactionManager;
        private readonly TaskControlService _taskControlService;

        // Inject the TransactionManager via constructor
        public GetGameDetailsTask(ITransactionManager transactionManager, TaskControlService taskControlService)
        {
        ...
        public async Task Invoke()
        {
            ...
            try
            {
                await _transactionManager.GetGameDetailsAsync(Payload);
            }
            catch (Exception ex)
            {
                Console.Error.WriteLine($"Error in GetGameDetailsTask: {ex.Message}");
            }
            finally
            {
