...
namespace Impostor.Plugins.SemanticAnnotator.Jobs {
    ...
    { 
        ...
        private readonly int _intervalMs; private CancellationTokenSource? _cts; 
        private Task? _backgroundTask; 
        
        public DecisionSupportScheduler(
            ILogger<DecisionSupportScheduler> logger, IServiceProvider serviceProvider)
        { 
            _logger = logger; _serviceProvider = serviceProvider; 
            _intervalMs = 5000; // puoi parametrizzare via config
        }
        public Task StartAsync(CancellationToken cancellationToken) { 
            _cts = new CancellationTokenSource(); _backgroundTask = RunAsync(_cts.Token); return Task.CompletedTask;
        }
        private async Task RunAsync(CancellationToken ct) { 
            ...
            while (await timer.WaitForNextTickAsync(ct)) {
                ...
                catch (Exception ex) { _logger.LogError(ex, "Error while executing DecisionSupportJob");
                }
            }
        }
