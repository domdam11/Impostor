...
{
    public class RemovePlayerTask : IInvocable, IInvocableWithPayload<(string gameId, string playerId, string description)>
    {
        private readonly ITransactionManager _transactionManager;
        private readonly TaskControlService _taskControlService;

        // Inject the TransactionManager via constructor
        public RemovePlayerTask(ITransactionManager transactionManager, TaskControlService taskControlService)
        {
        ...
        public async Task Invoke()
        {
            ...
            try
            {
                await _transactionManager.RemovePlayerAsync(Payload.gameId, Payload.playerId, Payload.description);
            }
            catch (Exception ex)
            {
                Console.Error.WriteLine($"Error in RemovePlayerTask: {ex.Message}");
            }
            finally
            {
